...
public class ConditionBuilderTest {
    ...
    private <X> void setupBuilder(Filter<X> filterInstance) {
        builder.buildFilterConditionForField(filterInstance, Column.create("time_to_go", Table.aliased("customer", "c")));
    }
}
...
class ConditionTestHelper {
    ...
    ConditionTestHelper(
        List<String> rangeFilterSetters,
        List<String> stringFilterSetters,
        List<String> generalFilterSetters,
        Boolean specified
    ) {
    ...
    public <X extends Comparable<? super X>> RangeFilter<X> buildRangeConditions(
        RangeFilter<X> filterInstance,
    ...
    public <X> Filter<X> buildGeneralConditions(Filter<X> filterInstance, @SuppressWarnings("unchecked") X... filterValue)
        throws IllegalAccessException, InvocationTargetException {
        invokeSetter(filterInstance, generalFilterSetters, specified, filterValue);
        return filterInstance;
    }

    private <X> void invokeSetter(
        Filter<X> filterInstance,
        List<String> fieldsToSet,
        Boolean specified,
        @SuppressWarnings("unchecked") X... filterValue
    ) throws IllegalAccessException, InvocationTargetException {
        for (String eachMethodName : fieldsToSet) {
            ...
            if (method.getName().toLowerCase().contains("setin") || method.getName().toLowerCase().contains("setnotin")) {
    ...
    private <X> Method getMethod(Filter<X> filterInstance, String eachMethodName) {
        return Arrays.asList(filterInstance.getClass().getMethods())
            .stream()
            .filter(eachMethod -> eachMethod.getName().equalsIgnoreCase(eachMethodName))
            .findFirst()
            .get();
    }
}
